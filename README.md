...
You can also do the other way around and use a shell script or command into a meshcore command, this is done prefixing with `<|` (for input pipe). Use `{}` or `{0}` as defaultplaceholder in your meshcli line to replace with the output of the shell command, you can specify your own key between `<` and `|` (and chain the input pipes) so
- `<name| whoami set name {name}`: will give your unix username to your node
